...
pub async fn outlook_oauth2_flow(app_handle: &AppHandle, timeout: u64) -> Result<String, String> {
    // Read Outlook credentials from config file
    let config_json = fs::read_to_string("outlook_client.json")
        .map_err(|e| format!("Failed to read outlook_client.json: {}", e))?;
    let config: OutlookConfig = serde_json::from_str(&config_json)
        .map_err(|e| format!("Failed to parse outlook_client.json: {}", e))?;
    
    let client_id = ClientId::new(config.client_id);
    let client_secret = ClientSecret::new(config.client_secret);
    let redirect_url = RedirectUrl::new("http://127.0.0.1:1426".to_string()).unwrap();
    let auth_url = AuthUrl::new("https://login.microsoftonline.com/common/oauth2/v2.0/authorize".to_string()).unwrap();
    let token_url = TokenUrl::new("https://login.microsoftonline.com/common/oauth2/v2.0/token".to_string()).unwrap();
    ...
    let (auth_url, _csrf_token) = client
        ...
        .add_scope(Scope::new("https://graph.microsoft.com/calendars.readwrite".to_string()))
    ...
    let token_result: oauth2::StandardTokenResponse<oauth2::EmptyExtraTokenFields, BasicTokenType> =
        client
            ...
            .await
            .map_err(|e| {
                println!("Outlook token exchange error: {:?}", e);
                e.to_string()
            })?;
    ...
    // Save tokens in a user-specific file
...
pub async fn outlook_oauth2_flow(_app_handle: &AppHandle, _timeout: u64) -> Result<String, String> {
    Err("Outlook OAuth is not supported on mobile platforms in this build.".to_string())
}