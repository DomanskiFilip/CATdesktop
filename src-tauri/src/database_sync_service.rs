...
impl DbSyncService {
    ...
    if let Err(e) = self.sync_to_dynamodb(app_handle, user_logged_in).await {
    ...
    if let Err(e) = self.sync_from_dynamodb(app_handle, user_logged_in).await {
    ...
    self.task_handle = Some(tokio::spawn(async move {
        ...
        let client = Client::new(); // Create a client outside the loop
        
        while running.load(Ordering::SeqCst) {
            interval.tick().await;
            
            // Create a temporary service for sync operations
            let temp_service = DbSyncServiceInner {
                config: config.clone(),
                client: client.clone()
            };
            
            // Each sync call has its own connection scope
            println!("Performing sync to DynamoDB...");
            if let Err(e) = temp_service.sync_to_dynamodb(&app_handle, logged_in).await {
                eprintln!("Sync failed: {}", e);
            }

            tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
            
            println!("Performing sync from DynamoDB...");
            if let Err(e) = temp_service.sync_from_dynamodb(&app_handle, logged_in).await {
                eprintln!("Sync from DynamoDB failed: {}", e);
            }
        }
    }));
    ...
    async fn send_to_dynamodb(&self, app_handle: &AppHandle, events: &[CalendarEvent]) -> Result<(), String> {
    ...
    async fn sync_to_dynamodb(&self, app_handle: &AppHandle, user_logged_in: bool) -> Result<(), String> {
        ...
        match self.send_to_dynamodb(app_handle, &events).await {
            Ok(_) => {
    ...
    pub async fn sync_from_dynamodb(&self, app_handle: &AppHandle, user_logged_in: bool) -> Result<(), String> {
        if !user_logged_in {
                ...
                if let Some(body) = lambda_response.get("body").and_then(|v| v.as_str()) {
                    if let Ok(body_json) = serde_json::from_str::<serde_json::Value>(body) {
                        if let Some(events_array) = body_json.get("events").and_then(|v| v.as_array()) {
                            ...
                            self.merge_remote_events(app_handle, events_array)
                                .await
...
impl Clone for AppConfig {
    fn clone(&self) -> Self {
        Self {
            ...
            // Clone other fields here...
        }
    }
}

// Helper struct for the background task
struct DbSyncServiceInner {
    config: AppConfig,
    client: Client,
}

// Implement the inner service methods
impl DbSyncServiceInner {
    // Helper method -> send events to DynamoDB //
    async fn send_to_dynamodb(&self, app_handle: &AppHandle, events: &[CalendarEvent]) -> Result<(), String> {
        // Prepare batch payload
        let payload = json!({
    ...
    // Helper method -> merge remote events into local database // BACKGROUND SERVICE //
    async fn merge_remote_events(&self, app_handle: &AppHandle, remote_events: &[Value]) -> Result<(), String> {
        let conn = get_db_connection(app_handle)
            .map_err(|e| format!("Database connection failed: {}", e))?;
    ...
    // Method to sync events to DynamoDB (upload changes) // BACKGROUND SERVICE //
    async fn sync_to_dynamodb(&self, app_handle: &AppHandle, user_logged_in: bool) -> Result<(), String> {
        // Verify user is actually logged in before proceeding
        if !user_logged_in {
            println!("User not logged in, skipping notification scheduling.");
        ...
        match self.send_to_dynamodb(app_handle, &events).await {
            ...
            Err(e) => {
                ...
                eprintln!("Failed to send events to DynamoDB: {}", e);
                Err(e)
            },
        }
    }
